...
- **Sources are live**: inline `[n]` markers link to the appendix row; appendix rows link out to the source URL. Source URLs are untrusted content — never embed them raw: HTML-escape every cell of the appendix table yourself, anchor each row (`id="src-n"`), and link only validated `http(s)` URLs (anything else renders as plain text; list it as an invalid URL). Apply the same escape discipline to any other source-derived text you place in attributes.
...
The `html_theme` knob in `config.toml` governs: a `file:` path loads a theme/brand spec to follow; inline text is applied as directives; empty means the shipped default — neutral, professional, generous whitespace, system font stack, one restrained accent color. Whatever the theme, the confidence-badge semantics above are non-negotiable.
