...
| Preset (`config.toml` `preset`) | subagents | sources/round | depth |
|---|---|---|---|
| `quick` | low (2) | 5 | 1 |
...
`subagent_models` is an ordered model preference for assistants — first available wins; empty means harness default. Keep the lead on the strongest model; researchers at most one tier down; judgment work never on the smallest tier.

## The plan gate

The one hard stop, kept light: decision, type and pack-derived dimensions pruned to it, shape, the **decomposition topology** — *breadth-first* (independent sub-questions: assistants split the dimensions), *depth-first* (one question that needs several perspectives: assistants split by angle or methodology, not by dimension), or *straightforward* (a focused ask: one assistant, a handful of calls, no fan-out — never overinvest in a simple query) — knobs in force and where each came from, which search surfaces exist (harness web search; installed search-shaped MCP tools; `external_sources` entries in `config.toml` — check, don't assume), whether to run the fan-out as a workflow when the harness offers orchestration and `use_workflows` allows, and an honest time estimate (a standard run is minutes; deep runs are tens of minutes and many times the tokens).

Present as a compact checklist, get approval, then: bind the run folder under the configured `research_output_path` — same deterministic folder name as Draft (type, topic slug, date) so the same topic always resolves to the same folder — seed `research.md` from the configured `research_template`, start `.memlog.md`, log the approved plan as a `decision` line, and tell the user the path.

Each dimension then runs in **rounds** — up to the resolved `max_depth` — and the report grows as material lands: the user watches the document build, not a spinner. Every digest is written to the run folder's `digests/` the moment it exists — one file per assistant per round (`<dimension>-r<round>-<n>.md`), the digest shape below, raw enough to re-derive from.
...
- its search surfaces (specialized tools first — installed search-shaped MCP tools, `external_sources` entries whose directive matches — then generic search), plus `preferred_sources` first / `banned_sources` never
...
**On each return, write the digest to the run folder's `digests/` before doing anything else with it.**

Spawn assistants on `subagent_models` when set (first available wins); otherwise the harness default — judgment work never drops to the smallest tier. When subagents are unavailable (or `subagents` is `none`), run the same rounds yourself, sequentially, under the same budgets and the same files-first discipline.
...
3. Log one `source` line in `.memlog.md` per source batch and one `claim` line per load-bearing claim worth tracking for refresh, in the machine-readable shape `ref=[n] status=<verified|unverified|disputed|overturned> class=<class> pub=<YYYY-MM> — <claim>`; a later status change is a fresh claim line with the same `ref=` (last status wins), so the ledger stays readable by a simple scan of the file.
