...
8. **Staleness map** — the claims that age fastest, computed not hand-derived: take the claims (`claim`, `class`, `pub_date`) from the ledger, map the pack's freshness bars to months per class, and compute each claim's re-check date (publication month plus its class's window) — render the re-check dates as a table and close by noting the earliest. This is Refresh's work order.

Update the frontmatter (`status: complete`, `updated`, and the verified/unverified/overturned counts read from the ledger's latest status per `ref=` in `.memlog.md` — never from memory), log a final `event` in the ledger, and proceed to `references/finalize.md`.
