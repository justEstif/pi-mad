...
The `.memlog.md` `claim` entries are the ledger: every claim a decision could rest on, with its class (each pack names its classes — quantitative sizes, pricing, versions/compatibility, regulatory assertions, …), source, publisher, publication date, and status. New claims enter `unverified`; on a Refresh or Deepen run, claims outside the run's scope keep their prior status from the ledger — only new and in-scope claims are (re)checked.
...
- **high** — cross-check every claim in the pack's *two-source classes*, and run the red-team pass on major conclusions regardless of the `red_team` knob.
...
Verifier assistants run behind the research firewall on `subagent_models` when set; judgment work never drops to the smallest tier.
