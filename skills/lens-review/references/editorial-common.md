...
The baseline style guide is `config.toml`'s `style_guide` (shipped default: Microsoft Writing Style Guide); a style guide stated in the request wins over the configured one for that run. Where the style guide in effect conflicts with a generic principle here — including the reader calibration — the style guide wins. Nothing overrides content being sacrosanct.

## Setup

1. Gather inputs: the content (required — a path or pasted text), plus whatever the request states: purpose, target audience, length target, reader type, style guide. If no reviewable content was provided, say so and stop. Request-level values win; `config.toml`'s `reader_type` (shipped default: `humans`) and `style_guide` fill what the request leaves unstated. Treat `config.toml`'s `review_guidance` entries as standing review directives.
2. When the content is a file, get exact word counts — document total and per heading section — via `python3 {skill-root}/scripts/word_metrics.py <path>` (`--help` documents the output), and ground every word-impact estimate and the reduction summary in those numbers. If the content was pasted or the script cannot run, estimate and mark the numbers as estimates.
...
Calibrate every finding to the reader type — stated in the request, else `config.toml`'s `reader_type`.
...
Shape the table per `config.toml`'s `output_preferences` (empty by default).
