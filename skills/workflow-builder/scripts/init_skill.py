...
A config.toml is emitted only when customization was accepted, never by default.
...
Usage:
  ...
  init_skill.py --name foo --dest DIR \
      --template /abs/SKILL-template.md --config-template /abs/config-template.toml
...
DEFAULT_CONFIG = SCRIPT_DIR.parent / "assets" / "config-template.toml"
...
def scaffold(args) -> dict:
    ...
    if args.customizable:
        ct_path = Path(args.config_template) if args.config_template else DEFAULT_CONFIG
        if not ct_path.is_file():
            raise FileNotFoundError(f"config template not found: {ct_path}")
        target = skill_dir / "config.toml"
        target.write_text(
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--customizable", action="store_true", help="emit config.toml (only when customization was accepted)")
    p.add_argument("--template", help="override path to the SKILL.md template")
    p.add_argument("--config-template", help="override path to the config.toml template")
