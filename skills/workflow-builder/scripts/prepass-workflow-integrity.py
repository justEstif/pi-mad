...
Extracts structural metadata from a skill that the LLM scanner
...
# Runtime variables that ARE expected (not artifacts). The builder resolves
# every placeholder at emit time, so this set is empty; any leftover {token}
# in a finalized skill is a template artifact.
RUNTIME_VARS = set()
