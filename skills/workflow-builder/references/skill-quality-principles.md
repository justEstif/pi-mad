...
```
## Resolution rules
- Bare paths (e.g. `references/press-release.md`) resolve from this skill's directory.
- Paths outside the skill resolve from the project working directory.
```

Additional rules:
- Forward slashes only (cross-platform).
- Values declared in config.toml that name paths are read from the file; never restate them as literals in SKILL.md.
...
config.toml is the only customization mechanism — no installer questions, no boolean-toggle config, no settings concept inside a built skill. The full spec (the ask, universal defaults, offered-when-relevant points, forbidden mechanisms) lives in `references/config-toml-guide.md`. The wiring rule worth carrying everywhere: SKILL.md must read declared values from the file — a hardcoded path beside a declared value silently no-ops the user's edit.
...
- The file uses the user's language (and produces its document in that language too).
...
When a skill is invoked non-interactively, the memlog absorbs every assumption made without the user: intent inference, proposed names, customization defaults, conflict resolutions, lint-fix calls, anything the user would have weighed in on interactively. Append these as typed `assumption` and `decision` entries as they happen, so the audit trail survives for the next session. Without this discipline, a non-interactive run silently buries its calls and the audit trail breaks on the next session.
...
- **Over budget** — a hard finding. Bring it back under budget through progressive disclosure: lift the largest self-contained section to `references/` or `assets/` and leave a one-line pointer, rather than compressing prose into something the model has to decode. Repeat until under the budget.

| File kind | Token budget |
| --- | --- |
| SKILL.md | desired aim / budget hard |
...
- **Three-mode architecture**: Guided, autonomous, non-interactive. Not every skill needs all three, but considering it during design prevents lock-in.
...
- **Boolean toggles in config.toml** → Author didn't decide what the skill does; the surface becomes a permutation forest. Fix: pick a default and let users fork if they want the other shape.
- **Hardcoded path in SKILL.md while config.toml declares the value** → Editing the file silently does nothing. Fix: SKILL.md must read the value from the file.
...
- **Working-state strategy buried under ceremony** → a memlog-discipline enumeration or a meta `## Workspace` section pays the pattern's cost without its value. Fix: thread it through the intents at the points that matter; a lean product-brief skill is the model.
