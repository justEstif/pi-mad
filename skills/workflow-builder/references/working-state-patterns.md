...
The memlog is typed, append-only, and lives as a `.memlog.md` file beside the primary artifact. The model never edits or re-reads it mid-session; it appends one typed entry at a time as its own line. The cycle is capture (append as decisions and directions land), distill (at finalize, account for every entry), and project (read the whole log once on resume or when building a summary).

### Entry format

Each entry is a single bullet line naming its type, then the text:

```
- [decision] Route validation to scripts/quick_validate.py, not an inline check
- [assumption] Output folder defaults to the project's skills/ directory
- [event] analyze: grade good, 0 critical / 1 high, report .analysis/<timestamp>/
```

`<type>` is one of `decision`, `direction`, `assumption`, `gap`, `note`, `event`. There is no edit or remove: history is never rewritten, a reversed decision gets a new entry that says so. When the work reaches its terminal state, append a final `event` entry saying so so a resumed session knows the run is complete.
