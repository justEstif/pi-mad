...
| Customization | `references/scan-customization.md` | `config.toml` surface economics, and confirmation that it is the only config mechanism present. |
| Enhancement | `references/scan-enhancement.md` | Missing named patterns to add and over-applied patterns to cut. |

Each lens returns the JSON in `references/lens-contract.md`. The leanness lens also returns `proposed_smallest` and `predicted_delta` on defend-against-absence findings, which you can route to a variant eval for a cut-or-keep verdict.

## Apply the config gates

Two gates from `config.toml` run alongside the lenses, only when configured:

- **`build_standards`** — if non-empty, check the skill against each directive (plain text or `file:` path) and fold any miss into the findings as a conformance finding.
- **`evals_required`** — if set, confirm the skill has the required evals (`"baseline"` or `"any"`); if not, add a high-severity finding.
...
```json
{
  ...
  "experience": {
    "journeys": [{ "name": "", "steps": "" }],
    "headless": "<one line on the skill's non-interactive story>"
  },
...
Append one memlog event carrying the grade (create `{target-skill-path}/.memlog.md` first if it does not exist):

```
- [event] analyze: grade <grade>, <c> critical / <h> high / <m> medium / <l> low, report .analysis/<timestamp>/skill-analysis-report.html
```

## Present

Present the grade, the one-line verdict, the severity tally, and the top themes. Point to the HTML report path, say it opened, and offer to walk through findings, apply a fix, or route a leanness finding's `proposed_smallest` to a variant eval.
