...
skill_dir comes from the adapter (default ".pi/skills"). Without this
...
references/platform-adapter.md, working examples in
assets/adapter-pi.json and assets/adapter-claude-code.json:
...
def find_adapter(explicit: Path | None, cases_file: Path) -> Path | None:
    ...
    env_path = os.environ.get("SKILL_EVALS_ADAPTER")
    if env_path and Path(env_path).is_file():
        return Path(env_path)
    for candidate in (
        cases_file.parent / "adapter.json",
        cases_file.parent / ".skill-evals-adapter.json",
    ):
...
def run_case(case: dict, case_dir: Path, run_dir: Path,
    ...
    if skill_path is not None:
        skills_subdir = (adapter or {}).get("skill_dir", ".pi/skills")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--adapter", type=Path, default=None,
                   help="adapter config JSON; defaults to SKILL_EVALS_ADAPTER env "
