...
def _make_repo(tmp_path):
    ...
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "work-1-1 initial a")
    (repo / "a.py").write_text("one\ntwo\nthree\nfour\n")
    (repo / "b.py").write_text("x\n")
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "work-1-2 grow a, add b")
    return repo
...
def test_measures_commits_and_files_with_attribution(tmp_path):
    repo = _make_repo(tmp_path)
    code, out = _run("--repo", str(repo), "--range", "HEAD~1..HEAD", "--units", "1-2,1-1")
    assert code == 0
    assert out["range"] == "HEAD~1..HEAD"
    assert out["commit_count"] == 1
    # The single commit in range is the second one; attributed to unit "1-2".
    assert out["commits"][0]["units"] == ["1-2"]
...
def test_unit_attribution_respects_word_boundary(tmp_path):
    # Unit id "1-2" must NOT match a commit subject mentioning "11-2".
    repo = tmp_path / "repo"
    repo.mkdir()
    _git(repo, "init", "-q")
    ...
    (repo / "f.py").write_text("a\nb\n")
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "work-11-2 unrelated unit")
    code, out = _run("--repo", str(repo), "--range", "HEAD~1..HEAD", "--units", "1-2")
    assert code == 0
    assert out["commits"][0]["units"] == []
...
def test_single_rev_range_rejected(tmp_path):
    ...
    # script would report the whole repo as the work's evidence.
    repo = _make_repo(tmp_path)
...
def _merge_repo(tmp_path):
    """Two unit branches merged into the mainline; the second merge conflicts
    and is resolved by hand, adding a line neither branch had. Returns
    (repo, base_sha) — `base_sha..HEAD` is the work range."""
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    (repo / "s.py").write_text("l1\nA\nl3\n")
    _git(repo, "commit", "-qam", "work-1-2 unit one")
    _git(repo, "checkout", "-q", "main")
    _git(repo, "merge", "-q", "--no-ff", "s1", "-m", "merge unit 1-2")

    _git(repo, "checkout", "-q", "-b", "s2", base)
    (repo / "s.py").write_text("l1\nB\nl3\n")
    _git(repo, "commit", "-qam", "work-1-3 unit two")
    _git(repo, "checkout", "-q", "main")
    conflicted = _git_unchecked(repo, "merge", "--no-ff", "s2", "-m", "merge unit 1-3")
    assert conflicted.returncode != 0, "fixture expected a merge conflict"
    (repo / "s.py").write_text("l1\nAB\nl3\nl4\n")  # hand resolution
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "merge unit 1-3")
    return repo, base
...
def test_merge_commits_listed_but_excluded_from_files(tmp_path):
    ...
    assert by_subject["merge unit 1-2"]["is_merge"] is True
    assert by_subject["merge unit 1-3"]["is_merge"] is True
    assert by_subject["work-1-2 unit one"]["is_merge"] is False
    # `files` is the two unit commits only — 1/1 each. Folding the merges in
...
def test_unit_attribution_survives_merges(tmp_path):
    repo, base = _merge_repo(tmp_path)
    out = _json(_proc("--repo", str(repo), "--range", f"{base}..HEAD", "--units", "1-2,1-3"))
    by_subject = {c["subject"]: c for c in out["commits"]}
    assert by_subject["work-1-2 unit one"]["units"] == ["1-2"]
    assert by_subject["work-1-3 unit two"]["units"] == ["1-3"]


def test_off_spine_merge_is_counted_but_not_measured(tmp_path):
    # A back-merge of the mainline into a unit branch is a merge in the range,
    # but its first-parent diff would restate unrelated mainline content as
    # work churn. It stays in merge_count and out of merges_measured, so the
    ...
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "work-1-2 unit one")
...
def test_repeated_merge_headers_are_counted_once(tmp_path):
    ...
    overlay = _fake_git(
        ...
        "    printf 'aaaa\\037p1 p2\\037merge unit 1-3\\n\\n2\\t1\\ts.py\\n"
        "aaaa\\037p1 p2\\037merge unit 1-3\\n\\n5\\t4\\ts.py\\n'\n"
        "    exit 0\n"
        "  fi\n"
        "done\n"
        f'exec "{real_git}" "$@"\n',
...
def test_valid_range_with_no_commits_keeps_the_full_shape(tmp_path):
    # A mis-specified work range is a valid, empty range. It must still answer
...
def test_second_pass_runs_only_when_the_range_has_merges(tmp_path):
    ...
    # or the unit-branch commits drop out of the listing and attribution dies.
...
def test_multi_story_subject_attributes_to_every_match(tmp_path):
    # First-match-wins silently dropped the second unit from the attribution.
    repo = tmp_path / "repo"
    repo.mkdir()
    _git(repo, "init", "-q", "-b", "main")
    (repo / "f.py").write_text("a\n")
    ...
    out = _json(_proc("--repo", str(repo), "--range", "HEAD~1..HEAD", "--units", "1-3,1-2"))
    # Every match, in --units order — not whichever id was passed first.
    assert out["commits"][0]["units"] == ["1-3", "1-2"]

    # A repeated id must not list the commit twice: any per-unit total built
    # from `units` would count it twice.
    repeated = _json(_proc("--repo", str(repo), "--range", "HEAD~1..HEAD", "--units", "1-3,1-2,1-3"))
    assert repeated["commits"][0]["units"] == ["1-3", "1-2"]
    assert repeated["units_supplied"] == ["1-3", "1-2"]
...
def test_subject_naming_no_story_gets_an_empty_list(tmp_path):
    ...
    out = _json(_proc("--repo", str(repo), "--range", "HEAD~1..HEAD", "--units", "1-2,1-3"))
    assert out["commits"][0]["units"] == []
...
def test_success_shape_carries_every_documented_key(tmp_path):
    ...
    assert set(out["commits"][0]) == {"sha", "subject", "units", "is_merge"}
