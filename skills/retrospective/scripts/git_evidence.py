            ...
            commits.append(
                {
                    "sha": sha,
                    "subject": subject,
                    # Every id the subject names, in --units order: a commit
                    # spanning two units belongs to both. Word-boundary match
                    # so a unit id like "1-2" does not also match "11-2".
                    "units": [sid for sid in units if re.search(rf"\b{re.escape(sid)}\b", subject)],
...
def main(argv=None):
    ...
    parser.add_argument(
        "--units",
        help="Comma-separated unit ids to match against commit subjects.",
    )
    ...
    if args.units:
        ...
        # would otherwise land twice in a commit's `units`, double counting
        # that commit in any per-unit total built from the output.
        units = list(dict.fromkeys(s.strip() for s in args.units.split(",") if s.strip()))
    ...
    # the range including merges, which is what per-unit attribution reads.
    # Merges contribute no numstat rows here, so `files` is non-merge churn.
    commits, files = _parse_log(_git_log(args.repo, [], args.range), units)
