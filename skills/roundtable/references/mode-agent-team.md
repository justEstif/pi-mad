...
Active when the session runs in `agent-team` mode (a `--mode agent-team` override). Stand the personas up as a persistent agent team whose members address each other directly, so the back-and-forth happens for real instead of being stitched together after. Only where the harness can stand up a persistent team — if it can't, fall back to `subagent`, and if that fails too, to `session`.
