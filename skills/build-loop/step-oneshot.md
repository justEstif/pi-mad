...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'full'` and `status: 'draft'`. Go back to `step-02-plan.md` step 6.

### Review

The active layers and their instructions are the `oneshot_review_layers` defaults in `config.toml`. Say which review layers you are skipping, then start every active layer before reading any results. Run them at the same time when you can. Fill in runtime placeholders first, and resolve every file path an instruction names to its absolute path (skill-directory paths resolve from this skill's directory). When a layer tells you to launch a reviewer subagent, launch it with that prompt text. Do not read the reviewer's instruction file yourself. For any other customized instruction, do what it says:

If a layer needs subagents and you cannot launch them, write the full prompt for each layer under `specs/` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `specs/deferred-work.md`:
...
Offer next steps in one line: create a PR (push first if needed) when git and a remote exist; use `change-walkthrough` to review the change with fresh eyes; or make another change.
...
If the `on_complete` default in `config.toml` is set, do it before exiting. Otherwise exit.
