...
2. Investigate the codebase. When you can, send deep searches to subagents and wait for them in this turn. Tell them to return short summaries only, so this session does not fill up with their notes. Keep only what the work needs: the specific files, symbols or lines, what to reuse, and what not to change. Write that into the Code Map. Do not retell the investigation when implementation starts — the spec already has it.
   ...
   Do not ask the human during investigation. When something is unclear, look in the repository, planning documents, or history first. Keep looking until you know, or until those sources have nothing more to say. Leave any remaining choice for the next step.
3. Run the route-selection rule — the `route_selection` default in `config.toml`.

   Intent gaps and irreversible steps (migrations, data mutation, external side effects) always take the full path below.

   For oneshot with intent resolved: read `spec-template.md` fully and write `{spec_file}`.
...
Before approving, you can open the spec file in an editor or ask me questions and tell me what to change. You can also use `pressure-test` (or another critique skill), ideally in another session to avoid context bloat.
...
- **Approve and stop** — approve the spec, leave it `ready-for-dev`, and stop so a fresh `build-loop` session can resume at implementation.
