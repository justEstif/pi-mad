...
The sanctum is the built agent's runtime memory, the place it reloads on every waking to become itself again, living in a memory directory under the project working directory (default `memory/{skillName}/`, created on first wake). This is a different thing from the builder's process log, the memlog, which is the builder's own trace written to `.memlog.md` beside the agent's SKILL.md while authoring. The two never blur. When this file or any file you write says memory of the sanctum, it means the agent's runtime memory and never the builder's log.
...
- When external skills are in play, suggest bundling them with the agent so it ships with its dependencies.
...
## config.toml is the sole config mechanism

An agent emits a config.toml beside SKILL.md only when the author opts in during build. It carries the override surface the author asked for (activation_steps_prepend, activation_steps_append, persistent_facts) as plain defaults the agent reads directly — no resolver, no override files, no merge step. The default is NO for memory and autonomous agents, because the sanctum is their customization surface; stateless agents are offered it once.

config.toml is the only build-time configuration surface an agent has. There is no other mechanism, and these are forbidden:

- No installer question that configures the agent.
- No settings or toggle concept baked into the built agent.
- No resolver, override-file layer, or merge logic — the values are plain defaults, edited in place.
- No identity, communication style, or principles in the config surface, because that content belongs in PERSONA, CREED, and BOND.

First Breath config and init-sanctum.py are a separate concern and are not build-time configuration. They initialize the agent's runtime sanctum the first time it wakes, which is runtime state, not the build surface. Any config.toml field that duplicates a sanctum concept is abuse, and First Breath must never be folded into config.toml.
