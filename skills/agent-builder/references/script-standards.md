...
**Always favor Python for script logic.** Bash is not portable — it fails or behaves inconsistently on Windows (Git Bash is MSYS2-based, not a full Linux shell; WSL bash can conflict with Git Bash on PATH; PowerShell is a different language entirely). Python works identically on all platforms.
...
Always prefer Python's standard library over external dependencies. The stdlib is pre-installed everywhere, requires no installs, and has zero supply-chain risk. Common stdlib modules that cover most script needs:
...
Every Python script MUST include a PEP 723 metadata block so its dependencies are declared in the file itself:
...
For scripts using only the standard library, the block is still present — it just lists no dependencies:
...
- Every declared dependency needs a graceful in-script fallback for when it is missing
...
- **All scripts:** `python3 scripts/foo.py {args}` — consistent invocation everywhere; the script itself declares any dependencies in its metadata block

## Graceful Degradation

Skills may run in environments where Python is unavailable (e.g., a hosted web runner). Scripts should be the fast, reliable path — but the skill must still deliver its outcome when execution is not possible.
