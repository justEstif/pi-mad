...
description: The Analyze orchestrator for agents. Runs the deterministic pre-pass, dispatches the quality lenses in parallel, merges their findings in-context, authors the synthesis layer, and renders the report deterministically via scripts/render_report.py. No per-subagent files.
---

**Language:** Use the user's language for all output.

# Analyze: Quality Analysis for an Agent
...
Each analyze run owns `{target-agent-path}/.analysis/<YYYY-MM-DD-HHmm>/` (create it first). It receives `findings.json`, `agent-analysis-report.html`, and `agent-analysis-report.md`. This run folder is the report location everywhere — the presented report paths point into it.

## Pre-scan check

Confirm the agent is resolvable at `{target-agent-path}` and that a `SKILL.md` is present. Note any uncommitted changes in the agent tree so the user knows the report reflects the working copy. You do not commit, stage, or push anything.
...
The lint scanner returns deterministic findings as JSON; carry its entries straight into the merged findings list with ids `lint-<n>`, keeping their severities. They are facts, not judgment, so no lens re-derives them.
...
| Customization | `references/scan-customization.md` | The config.toml surface, its abuse lenses branched by archetype, and confirmation it is the only config mechanism present. |
| Enhancement | `references/scan-enhancement.md` | Edge cases, experience gaps, delight, non-interactive potential, facilitative patterns. |
...
Every lens returns the JSON in `references/lens-contract.md`. Only the leanness lens fills `proposed_smallest` and `predicted_delta`; those two fields let you route a defend-against-absence finding to a variant eval for a real cut-or-keep verdict rather than a guess, and that routing happens in the build flow, not here.
...
Two config gates fold in here: if `build_standards` in this builder's `config.toml` is non-empty, check the agent against each directive (plain text or `file:` path) and add any miss as a conformance finding; if `evals_required` is set, confirm `{target-agent-path}/evals/cases.json` satisfies it (`"baseline"` or `"any"`) and add a high-severity finding when it does not.
...
- `agent_profile` — `name`, `title`, `icon`, `agent_type` (straight from the pre-pass), one-line `mission`. Drawn from the agent's identity as discovered.
...
- `sanctum` — only for memory and autonomous agents: `{ present: true, location, files, note }` where `location` is the agent's memory directory under the project working directory (default `memory/{skillName}/`) and `note` states that the sanctum is the built agent's runtime memory, distinct from the builder's `.memlog.md`. Omit the block (or set `present: false`) for a stateless agent.
- `experience` — `journeys` as `{ name, steps }` for the main paths a user takes through the agent, and `headless` as one line on the agent's non-interactive story.
...
- `standards` is always filled: resolve the three absolute paths from this builder's own directory at authoring time. The shell prepends them to every copied fix prompt, so the session that applies a fix holds the same bar that produced the findings.
