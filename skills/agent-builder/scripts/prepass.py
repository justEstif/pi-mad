...
`memory/{skillName}/` under the project working directory. An agent that reloads a sanctum on
...
def has_sanctum(root: Path, skill_text: str) -> bool:
    ...
    The sanctum is the built agent's memory under the project's `memory/` folder. We
    ...
    """
    if re.search(r"\bmemory/[\w.-]+/", skill_text):
        return True
