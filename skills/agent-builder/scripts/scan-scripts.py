#!/usr/bin/env python3
"""Deterministic scripts scanner for skill scripts.
...
def lint_python_ruff(filepath: Path, rel_path: str) -> list[dict]:
    ...
    if rc == -1:
        return [{
            ...
            'detail': '',
            'action': 'Ensure ruff is installed and runnable: ruff --version',
        }]
...
def lint_shell_shellcheck(filepath: Path, rel_path: str) -> list[dict]:
    ...
    if rc == -1:
        return [{
            ...
            'detail': '',
            'action': 'Ensure shellcheck is installed and runnable: shellcheck --version',
        }]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Scan skill scripts for quality, portability, agentic design, and lint issues',
    )
    parser.add_argument(
        'skill_path',
        type=Path,
