...
reads the agent's own config.toml (beside SKILL.md, optional) strictly to
substitute values into the sanctum templates, and it never writes or authors
any config file. Build-time customization is owned by config.toml, a surface
this script never touches.

Usage:
    python3 init-sanctum.py <project-dir> <skill-path>

    project-dir:  The project working directory (where memory/ lives)
...
def parse_config_values(config_path: Path) -> dict:
    """Simple key = "value" parser for the agent's config.toml.

    Handles top-level scalar string values only, which is all the sanctum
    templates need (user_name, communication_language)."""
    config = {}
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 init-sanctum.py <project-dir> <skill-path>")
        sys.exit(1)
    ...
    # Load config (the agent's own config.toml, beside SKILL.md; optional)
    config = parse_config_values(skill_path / "config.toml")
