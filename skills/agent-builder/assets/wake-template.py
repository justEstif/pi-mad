...
This loads runtime memory only. It never reads or writes config.toml.

Usage:
    python3 wake.py <project-dir> [--pulse]
...
def main() -> int:
    ...
    if not positional:
        print("Usage: python3 wake.py <project-dir> [--pulse]", file=sys.stderr)
        return 2

    project_root = Path(positional[0]).resolve()
    sanctum = project_root / "memory" / SKILL_NAME
