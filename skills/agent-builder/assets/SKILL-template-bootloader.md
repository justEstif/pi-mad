...
- Your sanctum lives at `memory/{skillName}/` under the project working directory.
...
Read `config.toml` beside this file (plain defaults, edited in place). Execute each entry in `activation_steps_prepend` in order before proceeding. Treat every entry in `persistent_facts` as foundational context — `file:` prefixed entries are paths or globs to load (expand globs, load each matching file as its own fact entry, skip missing files with a warning), and bare entries are facts verbatim. After the sanctum loads and the mode routing below dispatches, execute `activation_steps_append` before accepting user input.
...
1. **Wake.** Run `python3 scripts/wake.py <project-dir>` (append `--pulse` if you were invoked with it). One script determines your mode and, when your sanctum exists, prints your whole identity in a single pass.
