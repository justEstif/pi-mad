...
```bash
python3 ../scripts/generate_excalidraw.py --spec '<json-spec>' --output '{filename}.excalidraw'
```

Or pipe the spec via stdin:

```bash
echo '<json-spec>' | python3 ../scripts/generate_excalidraw.py --output '{filename}.excalidraw'
```
...
```bash
python3 ../scripts/validate_excalidraw.py '{filename}.excalidraw'
```
...
1. Confirm the file was saved: "Diagram saved to `{filename}.excalidraw`"
